...
class OnvifController:
            ...
            if (
                ...
                zoom_space_id = next(
                    (
                        ...
                        for i, space in enumerate(
                            getattr(
                                ptz_config.Spaces, "RelativeZoomTranslationSpace", None
                            )
                            or []
                        )
                ...
                try:
                    ...
                    # only relative zoom mode depends on this space; absolute and
                    # continuous zoom have their own capability checks below
                    if autotracking_config.zooming == ZoomingModeEnum.relative:
                        ...
                        logger.debug(
                            f"{camera_name}: Relative zoom space not available: {e}"
                        )
            else:
    ...
    async def _move_continuous_timed_with_zoom(
        ...
        if sunba_quirks:
            ...
            try:
                ...
                logger.warning(
                    f"ContinuousMove with zoom for {camera_name} failed: {e}"
                )
                self.cams[camera_name]["active"] = False
    ...
    async def _set_preset(self, camera_name: str, preset: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Updated preset '{preset}' to current position for {camera_name}"
            )
        except Exception as e:
    ...
    async def handle_command_async(
        ...
        try:
            if command == OnvifCommandEnum.init:
                ...
                return_preset = self.config.cameras[
                    camera_name
                ].onvif.autotracking.return_preset
                ...
                try:
                    if (
                        ...
                        self.ptz_metrics[camera_name].zoom_level.value = numpy.interp(
                            ...
                            [
                                self.cams[camera_name]["absolute_zoom_range"]["XRange"][
                                    "Min"
                                ],
                                self.cams[camera_name]["absolute_zoom_range"]["XRange"][
                                    "Max"
